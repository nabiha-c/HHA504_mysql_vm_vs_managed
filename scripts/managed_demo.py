# managed_demo.py — simplified demo for GCP Cloud SQL MySQL (Managed)
import os, time
...
# --- 0) Load environment ---
load_dotenv(".env")  # Make sure .env is in the project root
...
# --- 1) Connect to MySQL server (NO DB in URL) and ensure DB exists ---
server_url = f"mysql+pymysql://{MAN_DB_USER}:{MAN_DB_PASS}@{MAN_DB_HOST}:{MAN_DB_PORT}/"
print("[STEP 1] Connecting to Managed MySQL server:", server_url.replace(MAN_DB_PASS, "*****"))
t0 = time.time()
...
# --- 2) Connect directly to the target database ---
...
print("[STEP 3] Writing DataFrame to table on managed instance:", table_name)
...
# --- 4) Read back the row count ---
print("[STEP 4] Reading back row count from managed instance...")
